// ============================================================
// firebase-messaging-sw.js  —  UP ERP v3.1
// Universal Packaging Pvt. Ltd.
//
// BACKGROUND push notifications (tab closed / not focused).
// Rich notification format (reference image style):
...
const PRIO_DOT = { CRITICAL:'\u{1F534}', HIGH:'\u{1F7E0}', MEDIUM:'\u{1F7E1}', LOW:'\u{1F7E2}' };
const STAT_DOT = { OPEN:'\u{1F535}', ACCEPTED:'\u{1F7E3}', IN_PROGRESS:'\u{1F7E0}',
                   PENDING_REVIEW:'\u{1F7E1}', CONFIRMED:'\u{1F7E2}', LOCKED:'\u26AB', REJECTED:'\u{1F534}' };
...
function buildBody(d) {
  ...
  if (dept || mach) {
    lines.push('\uD83C\uDFED Dept: ' + (dept||'-') + '   \uD83D\uDD29 Machine: ' + (mach||'-'));
  }
  if (d.requestedBy) lines.push('\uD83D\uDC64 Requested By: ' + d.requestedBy);
  return lines.join('\n');
}
...
messaging.onBackgroundMessage(function(payload) {
  ...
  // Title: notifNo | Machine | Dept  (like reference image)
...
self.addEventListener('notificationclick', function(event) {
  ...
  // My Queue button OR body click OR any action → My Queue
  const tab   = (event.action === 'view_req' && reqId) ? 'requests' : 'queue';
  const hash  = tab === 'queue' ? '#maint-queue' : (reqId ? '#maint-req-' + encodeURIComponent(reqId) : '#maint-queue');
  const url   = (d.url || '/') + hash;

  event.waitUntil(
    clients.matchAll({ type:'window', includeUncontrolled:true }).then(function(list) {
      // If ERP tab already open — focus it and send message
      ...
      // No tab open — open new window (session auto-restored via localStorage)
...
self.addEventListener('push', function(event) {
  ...
  try {
    ...
    if (payload.notification) return; // Firebase handles it via onBackgroundMessage
    const d = payload.data || {};
    const parts = [d.notifNo||d.reqId, d.machine, d.dept].filter(Boolean);
    event.waitUntil(
      self.registration.showNotification(
        parts.join(' | ') || 'Maintenance Alert',
        { body: buildBody(d), icon:'/favicon.ico', data:{url:'/',reqId:d.reqId||'',tab:'queue'} }
      )
    );
  } catch(e) { console.log('[SW] Push parse error:', e.message); }
});

console.log('[SW] UP ERP firebase-messaging-sw.js v3.1 loaded');
